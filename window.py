...
def get_active_window_position():
    """
    parses xdtool output for position info
    returns: list[str]
    """
    root = subprocess.Popen(['xdotool','getwindowfocus','getwindowgeometry'], stdout=subprocess.PIPE)
    stdout, stderr = root.communicate()
    # Example: HDMI-1-1 connected 1920x1200+0+0 
    m = re.findall(b'Position: ([0-9]+),([0-9]+)', stdout)
    if m != None and len(m)>0:
        pos = [str(l.decode('utf-8')) for l in m[0]]
        return pos
    return []
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print(get_active_window_position())
        time.sleep(0.1)
